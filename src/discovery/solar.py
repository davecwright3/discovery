...
def theta_impact(psr):
    """From :py:mod:`enterprise_extensions.chromatic` : use the attributes of an Enterprise
...
def make_chromaticdelay(psr, idx=None):
    r"""Create a closure function for calculating chromatic exponential-dip delay.
    ...
    .. math:: -Ae^\left(-\Delta t / \tau\right) \left(f / 1400 \textrm{ MHz} \right)^{-\gamma}\ [\textrm{sec}].
       :label: exp-delay
